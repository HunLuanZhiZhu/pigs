...
Pigs is structured around two core layers: a \emph{proxy} layer that routes HTTP requests and a \emph{phased runtime} layer that orchestrates the Pre$\rightarrow$Executor$\rightarrow$Post state machine. A general-purpose agent client connects to the proxy via standard HTTP, using either \texttt{\{model\}} (passthrough) or \texttt{\{model\}-pig} (phased orchestration) as the model identifier. This section describes the core design of the proxy and runtime layers.
Pigs 围绕两个核心层构建：负责路由 HTTP 请求的\emph{代理}层和负责编排 Pre$\rightarrow$Executor$\rightarrow$Post 状态机的\emph{分阶段运行时}层。通用 Agent 客户端通过标准 HTTP 连接代理，使用 \texttt{\{model\}}（透传）或 \texttt{\{model\}-pig}（相位编排）作为模型标识符。本节描述代理和运行时层的核心设计。
...
\begin{itemize}[nosep]
    ...
    \texttt{/chat/completions} $\rightarrow$ OpenAI Chat
    \item \texttt{/v1/messages} $\rightarrow$ Anthropic Messages
    \texttt{/v1/messages} $\rightarrow$ Anthropic Messages
    \item \texttt{/responses} $\rightarrow$ OpenAI Responses
    \texttt{/responses} $\rightarrow$ OpenAI Responses
\end{itemize}
...
% Top row: Pre -> Executor -> Post -> Complete
...
% Budget exhaustion: Post -> Error (straight down)
