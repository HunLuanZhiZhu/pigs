...
该状态机由\emph{控制标记}驱动——模型附加到其输出中的简短文本标记（\texttt{PIGEND}、\texttt{PIGFAIL}）。这些标记由中间件检测，而非由模型自身的推理检测；它们充当路由信号，而非内容。关键在于，中间件会从可见输出中剥离这些标记，因此客户端只能看到模型的实质文本。与 ReAct 不同——在 ReAct 中模型可能跳过规划或反思——Pigs \emph{保证}每个非平凡请求都会经过全部三个阶段：该状态机无法被模型绕过。
...
\begin{enumerate}
    ...
    \textbf{消融研究}，在公开基准测试（SWE-bench、GAIA）上隔离各阶段（Pre、Executor、Post）对整体任务性能的贡献。
    ...
    \textbf{控制标记有效性研究}，验证基于 \texttt{PIGFAIL} 的故障恢复是否能改善结果，以及\textbf{三协议一致性评估}和\textbf{成本效益分析}，量化编排的令牌、延迟和 API 调用开销。
\end{enumerate}
...
本文的其余部分描述了架构（\Cref{sec:design}）、实现（\Cref{sec:impl}）、实验评估（\Cref{sec:experiments}）、相关工作（\Cref{sec:related}）和设计讨论（\Cref{sec:discussion}）。
